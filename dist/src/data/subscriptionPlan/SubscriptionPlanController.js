"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.create = exports.createSubscriptionPlan = exports.getSubscriptionPlan = exports.getAll = void 0;
const SubscriptionPlanData_1 = __importDefault(require("./SubscriptionPlanData"));
const getAll = (plan) => () => __awaiter(void 0, void 0, void 0, function* () {
    return plan.getAll();
});
exports.getAll = getAll;
const getSubscriptionPlan = (plan) => (id) => __awaiter(void 0, void 0, void 0, function* () {
    return plan.getById(id);
});
exports.getSubscriptionPlan = getSubscriptionPlan;
const createSubscriptionPlan = (plan) => (input) => __awaiter(void 0, void 0, void 0, function* () {
    return plan.create(input);
});
exports.createSubscriptionPlan = createSubscriptionPlan;
// export const updateProduct = (product: Data) => async (input?: CreateInput) => {
//   return product.update(input)
// }
function create(data) {
    return __awaiter(this, void 0, void 0, function* () {
        const plans = yield SubscriptionPlanData_1.default.create(data);
        return {
            getAll: (0, exports.getAll)(plans),
            getById: (0, exports.getSubscriptionPlan)(plans),
            create: (0, exports.createSubscriptionPlan)(plans),
            // update: updateProduct(products)
        };
    });
}
exports.create = create;
exports.default = { create };
//# sourceMappingURL=SubscriptionPlanController.js.map