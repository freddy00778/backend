...
export const resultsTemplate = async ({user_id, game_id, license_id}) => {
 ...
 const [
  ...
  (await UserProgressHandlers.create(data)).getSingleProgress({user: user_id, game: game_id}),
  (await UserHandlers.create(data)).getUser({id: user_id}),
  (await LicenseHandlers.create(data)).getById(license_id)
 ]);

 const currentActualEEScore = (progressObject?.actual_ee_score ? progressObject?.actual_ee_score : 0) + 100;
 const currentActualETScore = (progressObject?.actual_et_score ? progressObject?.actual_et_score : 0) + 100;
 const currentActualBudgetScore = (progressObject?.actual_budget_score ? progressObject?.actual_budget_score : 0) + 100;
 const overallScore = Math.floor((currentActualEEScore + currentActualETScore + currentActualBudgetScore) / 3);
 const licensedUser = await (await UserHandlers.create(data)).getUser({id: licenseObject?.user})

 const firstName = userObject?.firstName?.charAt(0).toUpperCase() + userObject?.firstName?.slice(1);
 const lastName = userObject?.lastName?.charAt(0).toUpperCase() + userObject?.lastName?.slice(1);
 ...
 // @ts-ignore
    ...
    <body>
      ...
      <p>
      <strong>
            Please contact your Administrator at ${licensedUser?.email}  if you have any questions or if you are interested in developing any specific Change Management skills.
    </strong>
       </p>
 ...
 <!DOCTYPE html>
    <html lang="en">
      ...
      <body>
        ...
        <footer class="footer">
          <p class="intro">
            Please contact your Administrator at ${licensedUser?.email} 
